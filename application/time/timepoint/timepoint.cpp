	...
	if ( can_register_in_lua< timepoint >( ngn_ptr ) )
		{
			auto type{ ngn_ptr->new_usertype< timepoint >(
				...
				sol::overload( []( const timepoint&		tp,
								   std::chrono::seconds sec ) { return timepoint( tp._timepoint + sec ); },
				...
				[]( const timepoint& a, const timepoint& b ) { return a._timepoint == b._timepoint; },
