...
static alerts_page::comparator compare_by_name{ []( const alert& lhs, const alert& rhs ) -> bool {
	return lhs.get_name() < rhs.get_name();
} };

static alerts_page::comparator compare_by_date{ []( const alert& lhs, const alert& rhs ) -> bool {
	return lhs.get_timepoint() < rhs.get_timepoint();
} };

static alerts_page::comparator compare_by_type{ []( const alert& lhs, const alert& rhs ) -> bool {
	return static_cast< int >( lhs.get_type() ) < static_cast< int >( rhs.get_type() );
} };

alerts_page::alerts_page( const std::string& name, const scripting::engine::ptr ngn_ptr, QWidget* parent )
	  ...
	  { .name		 = "_root_node",
		...
		.children	 = {
		  ...
		  _nd_t{
			{ .name = "remove",
			  .data
			  = { ._qaction = _bind_qaction_with_func( new QAction{ this }, [ this ]( auto remove_action ) {
					  auto con{ connect(
						  _lst_wgt,
						  &QListWidget::itemClicked,
						  [ this ]( QListWidgetItem* item ) {
							  if ( auto frame_ptr{ qobject_cast< QFrame* >( _lst_wgt->itemWidget( item ) ) } )
								  {
									  auto alert_ptr{ frame_ptr->property( "alert_ptr" )
														  .value< std::shared_ptr< alert > >() };
									  remove_alert( alert_ptr->get_name(), alert_ptr->get_alertist_name() );
									  sort_alerts();
								  }
						  } ) };
					  ...
					  auto exit_action{
						  new QAction{
									  QIcon( QPixmap{ std::string{ ":alerts_page/icons/exit.png" }.c_str() }
...
{
	if ( !std::any_of( _alerts.begin(), _alerts.end(), [ alert_ptr ]( const auto& elem_ptr ) {
			 return *elem_ptr == *alert_ptr;
		 } ) )
		{
...
void
alerts_page::remove_alert( const std::string& alert_name, const std::string& alertist_name )
{
	auto it{ std::find_if( _alerts.begin(), _alerts.end(), [ & ]( const auto& elem ) {
		return elem->get_name() == alert_name and elem->get_alertist_name() == alertist_name;
	} ) };
...
alerts_page::sort_alerts( std::optional< comparator > cmpr_opt )
{
	...
	std::sort( _alerts.begin(), _alerts.end(), [ cmpr = *cmpr_opt ]( const auto& lhs, const auto& rhs ) {
		return cmpr( *lhs, *rhs );
	} );
	...
	for ( const auto& alert_ptr : _alerts )
		{
			...
			switch ( alert_ptr->get_type() )
				{
					case alert::TYPE::INFO : styleSheet = "background:rgb(0, 29, 97); color: white;"; break;
					case alert::TYPE::WARNING :
						styleSheet = "background:rgb(121, 117, 0); color: white;";
						break;
					case alert::TYPE::ALARM : styleSheet = "background:rgb(102, 14, 0); color: white;"; break;
				}
			...
			switch ( alert_ptr->get_type() )
				{
					...
					case alert::TYPE::WARNING : iconPath = ":alerts_page/icons/warning.png"; break;
			...
			auto alertistLabel{ new QLabel{
			  QString( "Alertist: %1" ).arg( QString::fromStdString( alert_ptr->get_alertist_name() ) ) } };
			alertistLabel->setStyleSheet( "font-size: 10pt; font-style: italic; margin-top: 5px;" );
			...
			item->setSizeHint( QSize( frame->sizeHint().width(), frame->sizeHint().height() + 10 ) );
...
alerts_page::get_alerts()
{
	...
	for ( const auto& elem : _alerts ) { tmp.push_back( elem->make_lua_object_from_this() ); }
	return tmp;
}
	...
	if ( can_register_in_lua< alerts_page >( ngn_ptr ) )
		{
			...
			auto type{
				ngn_ptr->new_usertype< alerts_page >( _class_name, sol::base_classes, sol::bases< page >() )
			};
			type [ "get_alerts" ]	= &alerts_page::get_alerts;
			type [ "add_alert" ]	= &alerts_page::add_alert;
			type [ "remove_alert" ] = &alerts_page::remove_alert;
			type [ "sort_alerts" ]	= &alerts_page::sort_alerts; // TODO! is it possible to pass lua function
